...
public final class AppointmentMessageModel {
     ...
     */
    public AppointmentMessageModel(final String timeDescription, final Predicate<Appointment> timeCondition,
                                   final List<Appointment> allAppointments) {
        super();
        this.timeDescription = timeDescription;
        final List<Appointment> appointmentsAtTimeDescription = allAppointments.stream().filter(timeCondition)
                .collect(Collectors.toList());
        this.birthdays = appointmentsAtTimeDescription.stream().filter(Appointment::isBirthday)
                .collect(Collectors.toList());
     ...
     * @param components
     */
    private void enumerateAppointmentNamesAndAddMapping(final List<Appointment> appointmentsToEnumerate,
                                                        final List<String> components) {
        for (int i = 0; i < appointmentsToEnumerate.size(); i++) {
            final Appointment appointment = appointmentsToEnumerate.get(i);
            components.add(appointment.name());

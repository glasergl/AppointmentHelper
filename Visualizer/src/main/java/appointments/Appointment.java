...
public class Appointment implements Comparable<Appointment> {
	...
	public Appointment(final SimpleDate date, final String name, final boolean isABirthday)
			throws IllegalArgumentException {
	...
	public static Appointment jsonToAppointment(final JSONObject json) throws IllegalArgumentException {
		if (!representsAppointment(json)) {
			...
			return new Appointment(ofJSON, json.getString("name"), json.getString("description"),
					json.getBoolean("isABirthday"), json.getInt("id"));
		}
	}

	public static boolean representsAppointment(final JSONObject toTest) {
		return toTest.has("date") && toTest.has("name") && toTest.has("description") && toTest.has("isABirthday")
				&& toTest.has("id");
	}

 ...
 * @author glasergl
 */
public record SimpleDate(int day, int month) implements Comparable<SimpleDate> {
    /**
     * @param day
     * @param month
    ...
    public int compareTo(final SimpleDate toCompare) {
        final int monthCompareValue = Integer.compare(month, toCompare.month());
        if (monthCompareValue != 0) {
            return monthCompareValue;
        } else {
            return Integer.compare(day, toCompare.day());
        }
    }

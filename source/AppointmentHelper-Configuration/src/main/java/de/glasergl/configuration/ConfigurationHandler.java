...
public final class ConfigurationHandler {
	...
	try (final OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(configurationFile), StandardCharsets.UTF_8)) {
	    configuration.write(writer, DEFAULT_JSON_INDENT_FACTOR, DEFAULT_TOP_LEVEL_JSON_INDENT);
	} catch (final IOException e) {
	    throw new RuntimeException("Couldn't store configuration to the file at <" + configurationFile.getAbsolutePath() + ">");
	}
    }
	...
	try (final InputStreamReader reader = new InputStreamReader(new FileInputStream(configurationFile), StandardCharsets.UTF_8);) {
	    return new JSONObject(new JSONTokener(reader));
	} catch (final IOException e) {
	    throw new RuntimeException("Couldn't read configuration out of file at <" + configurationFile.getAbsolutePath() + ">");
	}
    }
     ...
     */
    public static File createConfigurationFile(final String pathOfNewConfigurationFile) throws IllegalArgumentException {
	...
	try (final OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(newConfigurationFile), StandardCharsets.UTF_8);) {
	    newConfiguration.write(writer, DEFAULT_JSON_INDENT_FACTOR, DEFAULT_TOP_LEVEL_JSON_INDENT);
	} catch (final IOException e) {
	    throw new RuntimeException("Couldn't create configurationFile at <" + newConfigurationFile.getAbsolutePath() + ">");
	}
